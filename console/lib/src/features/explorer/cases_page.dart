import 'package:arm_console/src/features/explorer/data/explorer_repository.dart';
import 'package:arm_console/src/features/explorer/domain/explorer_models.dart';
import 'package:arm_console/src/features/explorer/explorer_widgets.dart';
import 'package:arm_console/src/features/projects/domain/project_models.dart';
import 'package:arm_console/src/features/projects/project_controller.dart';
import 'package:arm_console/src/ui/console_primitives.dart';
import 'package:flutter/material.dart';
import 'package:go_router/go_router.dart';

class CasesExplorerPage extends StatelessWidget {
  const CasesExplorerPage({
    required this.projectController,
    required this.uri,
    this.repository = const AdaptiveExplorerRepository(),
    super.key,
  });

  final ProjectController projectController;
  final Uri uri;
  final ExplorerRepository repository;

  @override
  Widget build(BuildContext context) {
    return ListenableBuilder(
      listenable: projectController,
      builder: (BuildContext context, Widget? child) {
        final CasesQuery rawQuery = CasesQuery.fromUri(uri);

        return ConsolePageBody(
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: <Widget>[
              const ConsoleSectionHeader(
                eyebrow: 'Investigation',
                title: 'Cases',
                description:
                    'Individual occurrences stay visible with their own case IDs, evidence counts, follow-up references, and direct links back to the parent issue fingerprint.',
              ),
              const SizedBox(height: 24),
              _buildBody(context, rawQuery),
            ],
          ),
        );
      },
    );
  }

  Widget _buildBody(BuildContext context, CasesQuery rawQuery) {
    if (projectController.loadState == ProjectLoadState.loading) {
      return const SizedBox(
        height: 360,
        child: ConsoleStateView.loading(
          title: 'Loading case explorer',
          message: 'Preparing scoped cases and route-backed filters.',
        ),
      );
    }

    if (projectController.isTelemetryScopeLoading) {
      return const SizedBox(
        height: 360,
        child: ConsoleStateView.loading(
          title: 'Checking project access',
          message:
              'Verifying monitored-project telemetry permissions for the current scope.',
        ),
      );
    }

    if (projectController.loadState == ProjectLoadState.error) {
      return SizedBox(
        height: 360,
        child: ConsoleStateView.error(
          title: 'Case explorer unavailable',
          message:
              projectController.errorMessage ??
              'The case explorer could not be prepared.',
        ),
      );
    }

    final List<ConsoleProject> visibleProjects = projectController
        .telemetryProjects
        .toList();
    if (visibleProjects.isEmpty) {
      return SizedBox(
        height: 360,
        child: ConsoleStateView.noAccess(
          title: 'No readable projects',
          message:
              projectController.telemetryRestrictionMessage ??
              'Your session does not currently have any project scopes that can read monitored ARM telemetry.',
        ),
      );
    }

    final String? forcedProjectId = _forcedProjectId(visibleProjects);
    final bool hasInvalidProject =
        rawQuery.projectId != null &&
        !visibleProjects.any(
          (ConsoleProject project) => project.id == rawQuery.projectId,
        );
    final bool conflictsWithLockedScope =
        forcedProjectId != null &&
        rawQuery.projectId != null &&
        rawQuery.projectId != forcedProjectId;

    if (hasInvalidProject || conflictsWithLockedScope) {
      return SizedBox(
        height: 360,
        child: ConsoleStateView.noAccess(
          title: 'Requested case scope is not available',
          message:
              'The deep link requested a project scope that this session cannot inspect.',
          actionLabel: 'Clear project filter',
          onAction: () =>
              context.go(rawQuery.copyWith(clearProject: true).toLocation()),
        ),
      );
    }

    final CasesQuery effectiveQuery = forcedProjectId == null
        ? rawQuery
        : rawQuery.copyWith(projectId: forcedProjectId);

    return FutureBuilder<CasesResult>(
      future: repository.loadCases(
        query: effectiveQuery,
        visibleProjects: visibleProjects,
      ),
      builder: (BuildContext context, AsyncSnapshot<CasesResult> snapshot) {
        if (snapshot.connectionState != ConnectionState.done) {
          return const SizedBox(
            height: 360,
            child: ConsoleStateView.loading(
              title: 'Loading case explorer',
              message: 'Restoring the case table from the active route.',
            ),
          );
        }

        if (snapshot.hasError) {
          return SizedBox(
            height: 360,
            child: ConsoleStateView.error(
              title: 'Case query failed',
              message: 'The scoped case query could not be completed.',
              onAction: () => context.go(rawQuery.toLocation()),
            ),
          );
        }

        final CasesResult result = snapshot.data!;
        return Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: <Widget>[
            _buildFilters(
              context,
              rawQuery: rawQuery,
              effectiveQuery: effectiveQuery,
              visibleProjects: visibleProjects,
              projectLocked: forcedProjectId != null,
            ),
            const SizedBox(height: 16),
            if (result.issueFilterIssue != null) ...<Widget>[
              _IssueFilterCard(
                issue: result.issueFilterIssue!,
                onOpenIssue: () {
                  context.go(
                    IssuesQuery(
                      projectId: rawQuery.projectId,
                      range: rawQuery.range,
                      severityFilter: rawQuery.severityFilter,
                      search: result.issueFilterIssue!.fingerprint,
                      selectedIssueId: result.issueFilterIssue!.issueId,
                    ).toLocation(),
                  );
                },
                onClear: () {
                  context.go(
                    rawQuery.copyWith(clearIssueId: true).toLocation(),
                  );
                },
              ),
              const SizedBox(height: 16),
            ],
            if (result.selectedCase != null) ...<Widget>[
              _SelectedCaseCard(
                record: result.selectedCase!,
                onOpenIssue: () {
                  context.go(
                    IssuesQuery(
                      projectId: rawQuery.projectId,
                      range: rawQuery.range,
                      severityFilter: rawQuery.severityFilter,
                      search: result.selectedCase!.issueFingerprint,
                      selectedIssueId: result.selectedCase!.issueId,
                    ).toLocation(),
                  );
                },
                onOpenCaseDetail: () {
                  context.go('/cases/${result.selectedCase!.caseId}');
                },
                onClear: () {
                  context.go(
                    rawQuery.copyWith(clearSelectedCase: true).toLocation(),
                  );
                },
              ),
              const SizedBox(height: 16),
            ],
            if (result.totalCount == 0)
              SizedBox(
                height: 360,
                child: ConsoleStateView.empty(
                  title: 'No matching cases',
                  message:
                      'No cases matched the current project scope, parent issue filter, or search term.',
                  actionLabel: 'Clear filters',
                  onAction: () => context.go(const CasesQuery().toLocation()),
                ),
              )
            else
              _CasesTableCard(
                query: rawQuery,
                result: result,
                onOpenCase: (CaseRecord record) {
                  context.go('/cases/${record.caseId}');
                },
                onSelectCase: (CaseRecord record) {
                  context.go(
                    rawQuery.copyWith(selectedCase: record.caseId).toLocation(),
                  );
                },
                onOpenIssue: (CaseRecord record) {
                  context.go(
                    IssuesQuery(
                      projectId: rawQuery.projectId,
                      range: rawQuery.range,
                      severityFilter: rawQuery.severityFilter,
                      search: record.issueFingerprint,
                      selectedIssueId: record.issueId,
                    ).toLocation(),
                  );
                },
                onPreviousPage: result.currentPage > 1
                    ? () => context.go(
                        rawQuery
                            .copyWith(page: result.currentPage - 1)
                            .toLocation(),
                      )
                    : null,
                onNextPage: result.currentPage < result.totalPages
                    ? () => context.go(
                        rawQuery
                            .copyWith(page: result.currentPage + 1)
                            .toLocation(),
                      )
                    : null,
              ),
          ],
        );
      },
    );
  }

  Widget _buildFilters(
    BuildContext context, {
    required CasesQuery rawQuery,
    required CasesQuery effectiveQuery,
    required List<ConsoleProject> visibleProjects,
    required bool projectLocked,
  }) {
    final List<_ProjectOption> projectOptions = <_ProjectOption>[
      const _ProjectOption(id: null, label: 'All scoped projects'),
      ...visibleProjects.map(
        (ConsoleProject project) =>
            _ProjectOption(id: project.id, label: project.shellLabel),
      ),
    ];

    return ConsoleFilterBar(
      children: <Widget>[
        ExplorerSearchField(
          search: effectiveQuery.search,
          hintText: 'Search case ID, follow-up, or issue',
          onSubmitted: (String value) {
            context.go(
              rawQuery.copyWith(search: value.trim(), page: 1).toLocation(),
            );
          },
          onClear: () =>
              context.go(rawQuery.copyWith(search: '', page: 1).toLocation()),
        ),
        ExplorerMenuChip<ExplorerSeverityFilter>(
          label: 'Severity',
          value: effectiveQuery.severityFilter.label,
          items: ExplorerSeverityFilter.values,
          itemLabel: (ExplorerSeverityFilter item) => item.label,
          onSelected: (ExplorerSeverityFilter filter) {
            context.go(
              rawQuery.copyWith(severityFilter: filter, page: 1).toLocation(),
            );
          },
        ),
        ExplorerMenuChip<ExplorerDateRange>(
          label: 'Recency',
          value: effectiveQuery.range.label,
          items: ExplorerDateRange.values,
          itemLabel: (ExplorerDateRange item) => item.label,
          onSelected: (ExplorerDateRange range) {
            context.go(rawQuery.copyWith(range: range, page: 1).toLocation());
          },
        ),
        ExplorerMenuChip<CaseSort>(
          label: 'Sort',
          value: effectiveQuery.sort.label,
          items: CaseSort.values,
          itemLabel: (CaseSort item) => item.label,
          onSelected: (CaseSort sort) {
            context.go(rawQuery.copyWith(sort: sort, page: 1).toLocation());
          },
        ),
        if (projectLocked)
          ExplorerMenuChip<String>(
            label: 'Project',
            value: projectController.selectionLabel,
            items: const <String>[],
            itemLabel: (String item) => item,
            onSelected: (_) {},
            enabled: false,
          )
        else
          ExplorerMenuChip<_ProjectOption>(
            label: 'Project',
            value: projectOptions
                .firstWhere(
                  (_ProjectOption option) => option.id == rawQuery.projectId,
                  orElse: () => projectOptions.first,
                )
                .label,
            items: projectOptions,
            itemLabel: (_ProjectOption item) => item.label,
            onSelected: (_ProjectOption option) {
              context.go(
                option.id == null
                    ? rawQuery
                          .copyWith(clearProject: true, page: 1)
                          .toLocation()
                    : rawQuery
                          .copyWith(projectId: option.id, page: 1)
                          .toLocation(),
              );
            },
          ),
      ],
    );
  }

  String? _forcedProjectId(List<ConsoleProject> visibleProjects) {
    if (!projectController.canSelectAll ||
        !projectController.isAllProjectsSelected) {
      return projectController.selectedProjectId ?? visibleProjects.first.id;
    }
    return null;
  }
}

class _CasesTableCard extends StatelessWidget {
  const _CasesTableCard({
    required this.query,
    required this.result,
    required this.onOpenCase,
    required this.onSelectCase,
    required this.onOpenIssue,
    required this.onPreviousPage,
    required this.onNextPage,
  });

  final CasesQuery query;
  final CasesResult result;
  final ValueChanged<CaseRecord> onOpenCase;
  final ValueChanged<CaseRecord> onSelectCase;
  final ValueChanged<CaseRecord> onOpenIssue;
  final VoidCallback? onPreviousPage;
  final VoidCallback? onNextPage;

  @override
  Widget build(BuildContext context) {
    return ConsoleSurface(
      title: 'Recent cases',
      description:
          'Cases preserve issue linkage and follow-up identifiers so individual occurrences remain actionable even when routed in from a filtered issue view.',
      child: Column(
        children: <Widget>[
          SingleChildScrollView(
            scrollDirection: Axis.horizontal,
            child: DataTable(
              headingRowHeight: 52,
              dataRowMinHeight: 72,
              dataRowMaxHeight: 72,
              columns: const <DataColumn>[
                DataColumn(label: Text('Case ID')),
                DataColumn(label: Text('Issue')),
                DataColumn(label: Text('Project')),
                DataColumn(label: Text('Severity')),
                DataColumn(label: Text('Reported')),
                DataColumn(label: Text('Follow-up')),
                DataColumn(label: Text('Status')),
              ],
              rows: result.records.map((CaseRecord record) {
                final bool isSelected = record.caseId == query.selectedCase;
                return DataRow(
                  color: WidgetStateProperty.resolveWith<Color?>((
                    Set<WidgetState> states,
                  ) {
                    if (!isSelected) {
                      return null;
                    }
                    return Theme.of(
                      context,
                    ).colorScheme.primaryContainer.withValues(alpha: 0.35);
                  }),
                  cells: <DataCell>[
                    DataCell(
                      TextButton(
                        onPressed: () => onOpenCase(record),
                        style: TextButton.styleFrom(
                          padding: EdgeInsets.zero,
                          alignment: Alignment.centerLeft,
                        ),
                        child: Text(
                          record.caseId,
                          style: Theme.of(context).textTheme.labelLarge
                              ?.copyWith(fontWeight: FontWeight.w700),
                        ),
                      ),
                    ),
                    DataCell(
                      SizedBox(
                        width: 280,
                        child: TextButton(
                          onPressed: () => onOpenIssue(record),
                          style: TextButton.styleFrom(
                            padding: EdgeInsets.zero,
                            alignment: Alignment.centerLeft,
                          ),
                          child: Text(
                            record.issueFingerprint,
                            textAlign: TextAlign.start,
                          ),
                        ),
                      ),
                    ),
                    DataCell(
                      SizedBox(width: 180, child: Text(record.projectLabel)),
                    ),
                    DataCell(Text(record.severity.label)),
                    DataCell(Text(record.reportedLabel)),
                    DataCell(Text(record.followUpId)),
                    DataCell(
                      SizedBox(width: 140, child: Text(record.status.label)),
                    ),
                  ],
                );
              }).toList(),
            ),
          ),
          const SizedBox(height: 16),
          ExplorerPaginationFooter(
            totalCount: result.totalCount,
            currentPage: result.currentPage,
            totalPages: result.totalPages,
            onPrevious: onPreviousPage,
            onNext: onNextPage,
          ),
        ],
      ),
    );
  }
}

class _IssueFilterCard extends StatelessWidget {
  const _IssueFilterCard({
    required this.issue,
    required this.onOpenIssue,
    required this.onClear,
  });

  final IssueRecord issue;
  final VoidCallback onOpenIssue;
  final VoidCallback onClear;

  @override
  Widget build(BuildContext context) {
    return ConsoleSurface(
      title: 'Filtered to issue',
      description:
          'Cases can be opened from an issue deep link while keeping the parent fingerprint explicit and easy to clear.',
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: <Widget>[
          Text(
            issue.fingerprint,
            style: Theme.of(
              context,
            ).textTheme.titleMedium?.copyWith(fontWeight: FontWeight.w700),
          ),
          const SizedBox(height: 8),
          Text(issue.title),
          const SizedBox(height: 16),
          Wrap(
            spacing: 12,
            runSpacing: 12,
            children: <Widget>[
              FilledButton(
                onPressed: onOpenIssue,
                child: const Text('Open parent issue'),
              ),
              OutlinedButton(
                onPressed: onClear,
                child: const Text('Clear issue filter'),
              ),
            ],
          ),
        ],
      ),
    );
  }
}

class _SelectedCaseCard extends StatelessWidget {
  const _SelectedCaseCard({
    required this.record,
    required this.onOpenIssue,
    required this.onOpenCaseDetail,
    required this.onClear,
  });

  final CaseRecord record;
  final VoidCallback onOpenIssue;
  final VoidCallback onOpenCaseDetail;
  final VoidCallback onClear;

  @override
  Widget build(BuildContext context) {
    return ConsoleSurface(
      title: 'Selected case',
      description:
          'Specific case deep links preserve the occurrence in context while keeping the broader table state shareable.',
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: <Widget>[
          Text(
            record.caseId,
            style: Theme.of(
              context,
            ).textTheme.titleMedium?.copyWith(fontWeight: FontWeight.w700),
          ),
          const SizedBox(height: 8),
          Text(record.issueTitle),
          const SizedBox(height: 12),
          Wrap(
            spacing: 12,
            runSpacing: 12,
            children: <Widget>[
              _MetaPill(label: 'Project', value: record.projectLabel),
              _MetaPill(label: 'Severity', value: record.severity.label),
              _MetaPill(label: 'Reported', value: record.reportedLabel),
              _MetaPill(label: 'Follow-up', value: record.followUpId),
            ],
          ),
          const SizedBox(height: 16),
          Wrap(
            spacing: 12,
            runSpacing: 12,
            children: <Widget>[
              FilledButton(
                onPressed: onOpenCaseDetail,
                child: const Text('Open case detail'),
              ),
              FilledButton(
                onPressed: onOpenIssue,
                child: const Text('Open parent issue'),
              ),
              OutlinedButton(
                onPressed: onClear,
                child: const Text('Clear selection'),
              ),
            ],
          ),
        ],
      ),
    );
  }
}

class _MetaPill extends StatelessWidget {
  const _MetaPill({required this.label, required this.value});

  final String label;
  final String value;

  @override
  Widget build(BuildContext context) {
    final ColorScheme colorScheme = Theme.of(context).colorScheme;
    return Container(
      padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 10),
      decoration: BoxDecoration(
        color: colorScheme.surfaceContainerLow,
        borderRadius: BorderRadius.circular(14),
        border: Border.all(color: colorScheme.outlineVariant),
      ),
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        mainAxisSize: MainAxisSize.min,
        children: <Widget>[
          Text(
            label,
            style: Theme.of(context).textTheme.labelSmall?.copyWith(
              color: colorScheme.onSurfaceVariant,
            ),
          ),
          const SizedBox(height: 2),
          Text(
            value,
            style: Theme.of(
              context,
            ).textTheme.labelLarge?.copyWith(fontWeight: FontWeight.w700),
          ),
        ],
      ),
    );
  }
}

class _ProjectOption {
  const _ProjectOption({required this.id, required this.label});

  final String? id;
  final String label;
}
